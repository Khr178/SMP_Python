...
class Calculator:
    ...
    # Інкапсуляція(викликаю метод і він повертає результат, але всі обчислення приховані в методі.)
    def get_input(self):
        try:
...
#Поліморфізм(перевіряє оператори, якщо додати новий оператор, код не зміниться)
